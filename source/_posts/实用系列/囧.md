...
flowchart LR
    ...
    J-->X["下单模块(下单者)"]-->创建新订单
    J-->L["订单管理模块"]-->VV["查看历史订单(下单者/接单者/管理员)"] & V[查看每一个订单]
    V-->Q["取消订单(下单者)"] & QQ["标记订单已完成(下单者)"] & QQQ["接单(接单者)"] & QQQQ["取消接单(接单者)"]
    J-->F[订单分配模块]-->分配订单给相近地址的接单者
    J-->G[个人信息管理模块]-->查看个人信息 & 修改个人信息 & 注销登录 & 注销账号
```
...
1. 浏览器访问某个页面/点击某一个组件
2. 全局前置守卫判断该页面是否需要认证，如果需要认证，判断 localStorage 里有没有 token。如果不需要认证或者 localStorage 里有 token，放行。如果需要认证且 localStorage 里没有 token，清除 token 和 axios 请求头的 Authorization 字段，并导航到 /auth
3. 某个组件挂载之后发请求，或点击组件发请求之后，axios 请求拦截器检查 localStorage 里有没有 token，如果有，设置为请求头的 Authorization 字段。如果没有，导航到 /auth
...
7. 其他 /api 下的请求都会被后端拦截，验证 Token 表里有没有该 token 与其对应的手机号。如果没有，返回认证失败。如果有，向 UserContext 类里写手机号和 token，后续 Controller 类再获取该对象，调用 UserService.who 根据 User 手机号查身份
...
|               | role -> customer/provider/admin | sex -> 男              |
|               |                                 | age -> 23              |
|               |                                 | adcode -> 420112       |
...
|                | order_time -> 64 位时间戳 | address -> 湖北省武汉市东西湖区            | acceptance_time -> 64 位时间戳 |
|                |                           | adcode -> 420112                           |                                |
|                |                           | description -> 服务描述                    |                                |
...
### GET /api/order/{id} 获取指定 ID 的订单 200 | 404
...
下单者/接单者获取与本人关联的。管理员获取所有的。没有也返回一个空的。
