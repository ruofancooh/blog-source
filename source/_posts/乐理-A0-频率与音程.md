...
# 生成钢琴的88个音
pks = piano_keys = ["A0", "#A0", "B0"]
pks.extend([key + str(i) for i in range(1, 8) for key in oks])
pks.extend(["C8"])
pks = tuple(pks)


def calc_frequency(alphabet: str) -> float:
    """
    计算输入音名的频率，音名需要在88键钢琴内
    返回频率
    """
    # 基准频率
    F_A4 = 440
    # 计算相隔的半音数
    semitones = pks.index(alphabet) - pks.index("A4")
    # 计算频率
...
def create_frequency_dict() -> None:
    """
    生成频率字典
    """
    for key in pks:
        frequency_dict[key] = calc_frequency(key)


while True:
    alphabet = input("输入音名：")
    if alphabet in pks:
        f_alphabet = calc_frequency(alphabet)
