...
class PianoKeyFreq:
    pianokey_freq: dict  # 存放音名频率的字典

    def __init__(self):
        """
        生成音名频率字典
        """
        self.pianokey_freq = dict()
        for key in PIANO_KEYS:
            self.pianokey_freq[key] = self.calc_frequency(key)

    def calc_frequency(self, alphabet: str) -> float:
        """
        计算音名的频率，音名需要已经在88键钢琴内
        返回频率
        """
        # 基准频率
        F_A4 = 440
        # 计算相隔的半音数
        semitones = PIANO_KEYS.index(alphabet) - PIANO_KEYS.index("A4")
        # 计算频率
        f_alphabet = F_A4 * 2 ** (semitones / 12)
        return f_alphabet


class PrintPianoKeyFreq(PianoKeyFreq):
    def print_freq(self, alphabet: str) -> float:
        """
        输出一个音名对应的频率
        """
        try:
            f_alphabet = self.pianokey_freq[alphabet]
            print(f"{alphabet} 的频率是 {f_alphabet:.3f}Hz")
        except:
            print("找不到音名，输入 all 以查看所有音名的频率")

    def print_all_freq(self) -> None:
        """
        输出所有音名对应的频率
        """
        for k, v in self.pianokey_freq.items():
            print(f"{k:>3} 的频率是 {v:.3f}Hz")


while True:
    ppkf = PrintPianoKeyFreq()
    inpu = input("输入音名：")
    if inpu == "all":
        ppkf.print_all_freq()
    else:
        ppkf.print_freq(inpu)
    print()
```
