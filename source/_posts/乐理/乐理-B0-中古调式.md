...
$\mathrm{C-D-E-F-G-A-B}$ 和 $\mathrm{A-B-C-D-E-F-G}$ 音阶相同吗？
...
- 其次，音级之间的音程关系（相隔的半音数）不同。
  前者为 $2-2-1-2-2-2-1$，后者为 $2-1-2-2-1-2-2$。（算上下一个八度里的主音）
...
- 以 $\mathrm{C}$ 为主音，音程关系为 $(2-2-1-2-2-2-1)$ 的调式，称为 $\mathrm{C}$ **自然大调**。`C Major`
  - 用到的音名为：$\mathrm{...B3-C4-D4-E4-F4-G4-A4-B4-C5...}$
- 以 $\mathrm{A}$ 为主音，音程关系为 $(2-2-1-2-2-2-1)$ 的调式，称为 $\mathrm{A}$ **自然大调**。`A Major`
  - 用到的音名为：$\mathrm{...G\sharp3-A4-B4-C\sharp4-D4-E4-F\sharp4-G4\sharp-A5...}$
- 以 $\mathrm{A}$ 为主音，音程关系为 $(2-1-2-2-1-2-2)$ 的调式，称为 $\mathrm{A}$ **自然小调**。`A Minor`
  - 用到的音名为：$\mathrm{...G3-A4-B4-C4-D4-E4-F4-G4-A5...}$
...
| --------------------------------- | ------------------------------------- | --------------- |
| 伊奥尼亚调式（Ionian mode）       | **自然大调**`Major`、中国七声清乐宫调 | `2-2-1-2-2-2-1` |
| 多利亚调式（Dorian mode）         | 中国七声清乐商调                      | `2-1-2-2-2-1-2` |
| 弗里几亚调式（Phrygian mode）     | 中国七声清乐角调                      | `1-2-2-2-1-2-2` |
| 利底亚调式（Lydian mode）         |                                       | `2-2-2-1-2-2-1` |
| 混合利底亚调式（Mixolydian mode） | 中国七声清乐徵调                      | `2-2-1-2-2-1-2` |
| 爱奥尼亚调式（Aeolian mode）      | **自然小调**`Minor`、中国七声清乐羽调 | `2-1-2-2-1-2-2` |
| 洛克里亚调式（Iocrian mode）      |                                       | `1-2-2-1-2-2-2` |
...
在实践演奏中，并没有一开始就规定七个音级、谁作主音。后来为了总结经验，方便记录，把演奏过程中用到次数最多的音、最主要的音称为“主音”，然后发展出调式理论。

## 作业

用 Python 写一个程序，输入 $7 \times 12$ 种任意中古调式的名字，输出对应的七个音级的音名。

**输入要求：**

不分大小写，升降号写在音名前后都可以，但是音名 $\mathrm{B}$ 必须大写，为了和降号`b`区分。
...
字母`A B C D E F G`都出现且只出现一次，在音名后面适当加升降号。
